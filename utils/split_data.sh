...
if [ ! -d $s1 ]; then 
    ...
    need_to_split=false
    for f in utt2spk spk2utt spk2warp feats.scp text wav.scp cmvn.scp cmvn_utt2utt.scp cmvn_skp2utt.scp spk2gender \
...
for n in `seq $numsplit`; do
    ...
    for f in spk2gender spk2warp cmvn.scp cmvn_spk2utt.scp; do
        [ -f $data/$f ] && \
            utils/filter_scp.pl $dsn/spk2utt $data/$f > $dsn/$f
    done

    # filter utt2utt and  cmvn_utt2utt.scp from feats.scp
    for f in  utt2utt cmvn_utt2utt.scp; do
        [ -f $data/$f ] && \
            utils/filter_scp.pl $dsn/feats.scp $data/$f > $dsn/$f
    done
